...
const addTrack = (track, streamId) => {
  if (track.id && !trackRecorders?.[track.id]) {
    ...
    trackRecorders[track.id].ondataavailable = async (event) => {
      ...
      chrome.runtime.sendMessage(extensionId, message={action: 'uploadFile', data: raw, streamId: streamId, trackId: track.id, kind: track.kind});
    }
    // send message to puppeteer
    sendMessageToPuppeteer(message = {action: 'AVTracksAdded', streamId: streamId, trackId: track.id, kind: track.kind});
  ...
  window.RTCPeerConnection = function() {
    ...
    pc.addEventListener('track', function(e) {
      ...
      e.track.addEventListener('mute', (ev) => {
        trace('ontrackmuted', e.track.kind + " muted: "+ e.track.muted + " enbled: " + e.track.enabled + ':' + e.track.id);
      });
      ...
      trace('ontrack', e.track.kind + ": "+ e.track.muted + ": " + e.track.enabled + ':' + e.track.id + ' ' + e.streams.map(function(s) { return 'stream:' + s.id; }));
    });
    return pc;
  };
